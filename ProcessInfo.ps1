...
if ($ProcessInfo) {
    ...
    foreach ($Thread in $ActiveThreads) {
        ...
        $Stack = Trace-Thread -ProcessHandle $ProcessHandle -ThreadId $Thread.IDThread -ProcessId $Thread.IDThread

        <#Note:
        Start address in process explorer is represented by symbolic name (module) associated with the program-specified location...where the thread begam executing
        Source: Book: Troubleshooting with Windows Sysinternals Tools - Process Explorer Thread Details (page 97)
        Thoughts:  I interpret this as 3rd element in the call stack 
        #>

        # step through each item in stack in reverse order
        $Start = $Stack[-3]
        ...
        {
            0 { $ThreadState = 'Initialized'    } # A state that indicates the thread has been initialized, but has not yet started.
            1 { $ThreadState = 'Ready'    }       # A state that indicates the thread is waiting to use a processor because no processor is free. The thread is prepared to run on the next available processor.
            2 { $ThreadState = 'Running'   }      # A state that indicates the thread is currently using a processor.
            3 { $ThreadState = 'Standby' }        # A state that indicates the thread is about to use a processor. Only one thread can be in this state at a time.
            4 { $ThreadState = 'Terminated'  }    # A state that indicates the thread has finished executing and has exited.
            5 { $ThreadState = 'Wait'    }        # A state that indicates the thread is not ready to use the processor because it is waiting for a peripheral operation to complete or a resource to become free. When the thread is ready, it will be rescheduled.
            6 { $ThreadState = 'Transition'    }  # A state that indicates the thread is waiting for a resource, other than the processor, before it can execute. For example, it might be waiting for its execution stack to be paged in from disk.
            7 { $ThreadState = 'Unknown'  }       # The state of the thread is unknown.

        }
        switch ( $Thread.ThreadWaitReason )
        {
            0 { $ThreadWaitReason = 'Executive'    }      # The thread is waiting for the scheduler.
            1 { $ThreadWaitReason = 'FreePage'    }       # The thread is waiting for a free virtual memory page.The thread is waiting for a free virtual memory page.
            2 { $ThreadWaitReason = 'PageIn'   }          # The thread is waiting for a virtual memory page to arrive in memory.
            3 { $ThreadWaitReason = 'SystemAllocation' }  # The thread is waiting for system allocation.
            4 { $ThreadWaitReason = 'ExecutionDelay' }    # Thread execution is delayed.
            5 { $ThreadWaitReason = 'Suspended'    }      # Thread execution is suspended.
            6 { $ThreadWaitReason = 'UserRequest'    }    # The thread is waiting for a user request.
            7 { $ThreadWaitReason = 'EventPairHigh'  }    # The thread is waiting for event pair high.
            8 { $TheeadWaitReason = 'EventPairLow' }      # The thread is waiting for event pair low.
            9 { $TheeadWaitReason = 'LpcReceive' }        # The thread is waiting for a local procedure call to arrive.
            10 { $heeadWaitReason = 'LpcReply' }         # The thread is waiting for reply to a local procedure call to arrive.
            11 { $TheeadWaitReason = 'VirtualMemory' }    # The thread is waiting for the system to allocate virtual memory.
            12 { $TheeadWaitReason = 'PageOut' }          # The thread is waiting for a virtual memory page to be written to disk.
            13 { $TheeadWaitReason = 'Unknown' }          # The thread is waiting for an unknown reason.
        }
